...
const DhakaChamber = () => {
   ...
   const dd = fullName.replaceAll("-"," ")
   const abc = DhakaData.find(pd=>pd.name===dd)
   console.log(dd)

   const {name,Education,Specialist,workPlace,Desination,ChamberAddress,ChamberName,phone,image,} = abc
    return (
        <>
         <title>{name}</title>
      <title>{ChamberName}</title>
